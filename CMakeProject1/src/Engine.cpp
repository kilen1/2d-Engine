...
namespace CmakeProject1 {
    ...
    bool Engine::initialize(int argc, char* argv[]) {
        ...
        if (!initializeSDL() || !initializeOpenGL() || !initializeSystems() || 
            !initializeTools() || !initializeScripting()) {
            return false;
        }
    ...
    bool Engine::initializeSystems() {
        ...
        m_window = std::make_unique<class Window>();
        m_renderer = std::make_unique<class Renderer>();
        m_inputManager = std::make_unique<class InputManager>();
        m_assetManager = std::make_unique<class AssetManager>();
        m_sceneManager = std::make_unique<class SceneManager>();
        m_projectManager = std::make_unique<class ProjectManager>();
        m_scriptEngine = std::make_unique<class ScriptEngine>();
        m_physicsEngine = std::make_unique<class PhysicsEngine>();
        m_audioEngine = std::make_unique<class AudioEngine>();
        m_eventSystem = std::make_unique<class EventSystem>();
        m_timeManager = std::make_unique<class TimeManager>();
    ...
    bool Engine::initializeTools() {
        ...
        m_graphicsTablet = std::make_unique<class GraphicsTablet>();
        m_toolManager = std::make_unique<class ToolManager>();
        
        // Initialize UI tools
        m_contentBrowser = std::make_unique<class ContentBrowser>();
        m_console = std::make_unique<class Console>();
        m_profiler = std::make_unique<class Profiler>();
        m_hierarchy = std::make_unique<class Hierarchy>();
        m_inspector = std::make_unique<class Inspector>();
        m_sceneViewer = std::make_unique<class SceneViewer>();
        m_gameView = std::make_unique<class GameView>();
    ...
    void Engine::render() {
        ...
        // Render editor if enabled
        if (m_editor && m_editorEnabled) {
            renderEditor();
        }
        
        // Swap buffers (assuming we're using SDL_GL_SwapBuffers)
        if (m_window) {
